...
if CONFIG_DATA_TYPE==CONFIG_DATA_TYPE_FP32:
    ...
    A=np.random.randint(0, high=100, size=(d+0,d+1,d+2,d+3))
    B=np.random.randint(0, high=100, size=(d+0,d+1,d+3,d+2))
else:
...
if CONFIG_UNIFORM_DIM:
    if CONFIG_DATA_TYPE==CONFIG_DATA_TYPE_FP32:
        ...
        A=np.random.randint(0, high=100, size=(d,d,d,d))
        B=np.random.randint(0, high=100, size=(d,d,d,d))
    else:
        print("Data type not specified....")
        exit(1)
...
if ENABLE_CONTRACTION:
    print("A, Ap, App (shape): ", A.shape, Ap.shape, App.shape)
    print("B, Bp, Bpp (shape): ", B.shape, Bp.shape, Bpp.shape)
